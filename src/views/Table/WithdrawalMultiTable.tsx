...
import {
  useWithdrawalPendingMultiQuery,
  Withdrawal,
  Pagination,
} from "@/hooks/queries/backoffice/withdrawal/pending/multi";
...
const WithdrawalMultiTable = (props: Props) => {
  ...
  const { data, isLoading, isError } = useWithdrawalPendingMultiQuery({
    menutype,
    ...pagination,
  });
  ...
  const handleRowClick = (row: Withdrawal) => {
    router.push(`/multi/${type}/request-withdrawal/${row.withdrawal_id}`);
  };
